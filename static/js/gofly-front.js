...
GOFLY.getNotice=function(){
    ...
    $.get(this.GOFLY_URL+"/notice?kefu_id="+this.GOFLY_KEFU_ID,function(res) {
        if(res.result.status=='offline'){
            ...
            setTimeout(function(){
                var userInfo="<img style='margin-top: 5px;' class='flyAvatar' src='"+_this.GOFLY_URL+res.result.avatar+"'/> <span class='flyUsername'>"+res.result.nickname+"</span>"
                $('.launchButtonText').html(userInfo);
            },3000);
        }
        _this.kefuAvator=res.result.avatar;
        _this.kefuName=res.result.nickname;
        _this.chatPageTitle+="<img src='"+_this.GOFLY_URL+res.result.avatar+"' class='flyAvatar'>"+res.result.nickname;
        ...
        if (res.result.welcome != null) {
            ...
            if(len>0){
                ...
                _this.noticeTimer=setInterval(function(){
                    ...
                    var welcomeHtml="<div class='flyUser'><img class='flyAvatar' src='"+_this.GOFLY_URL+res.result.avatar+"'/> <span class='flyUsername'>"+res.result.nickname+"</span>" +
                        "<span id='launchNoticeClose' class='flyClose'>×</span>" +
                        "</div>";
